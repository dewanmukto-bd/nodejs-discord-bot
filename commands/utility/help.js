...
module.exports = {
    ...
    try { let invite = await message.channel.createInvite({maxAge: 10 * 60 * 1000, maxUses: 5 });
        ...
        const info_row = new MessageActionRow()
            .addComponents(
                new MessageButton().setLabel('Website').setURL('https://muxday.muxsites.com/discordbot/').setStyle('LINK'),
                new MessageButton().setLabel('Upgrade').setURL('https://patreon.com/MuxPatreon').setStyle('LINK'),
                new MessageButton().setLabel('Contact').setURL('https://discord.com/invite/RUVPF75').setStyle('LINK').setDisabled(true),
        ...
        if(!args[0]) {
            ...
            embed.setFooter({ text: ` MuxDay \t Total commands loaded: \`${client.commands.size}\``, iconURL: client.user.displayAvatarURL({ dynamic: true })});
		embed.addField('🎵️ **Audio** (30+)',"All audio commands have moved to [MuxMusic](https://muxworks.com/musicbot)")
